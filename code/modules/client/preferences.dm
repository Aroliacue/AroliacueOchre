...
//						age = max(min( round(text2num(new_age)), AGE_MAX),AGE_MIN)
				...
				if("ooc_extra")
					...
					var/new_extra_link = tgui_input_text(user, "Input the accessory link (https, hosts: catbox, file garden):", "Song URL", ooc_extra, max_length = MAX_MESSAGE_LEN, encode = FALSE) //OV EDIT - Discord Removed
